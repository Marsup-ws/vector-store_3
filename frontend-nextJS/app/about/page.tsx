...
export default function AboutSectionCopy() {
  ...
  return (
    /* CHANGEMENT 1: items-start au lieu de items-center pour aligner en haut */
    <section className="relative min-h-screen w-full rounded-3xl flex items-start justify-center p-4 sm:p-8 md:p-12 bg-[#FBF7FB] text-gray-800 overflow-hidden font-serif">
      ...
      <div className="relative w-full max-w-6xl rounded-3xl bg-gradient-to-br from-[#FFF8FA] via-[#F6EEFA] to-[#EDE7F6] p-6 sm:p-10 lg:p-14 shadow-xl border border-pink-100/50 overflow-hidden">
        ...
        >
          <div className="relative h-full max-h-[85vh] aspect-[880/900] opacity-100 mix-blend-multiply flex items-center justify-end pr-2 sm:pr-6 lg:pr-8">
            <Image
              ...
              height={900}
              priority
              /* CHANGEMENT 2: rounded-2xl (ou rounded-3xl) pour arrondir les bords de l'image */
              className="h-full w-auto max-h-[85vh] object-contain object-right filter contrast-[1.05] rounded-2xl"
              sizes="(max-width: 768px) 100vw, 50vw"
            />
          </div>
        </motion.div>

        {/* CHANGEMENT 3: max-w-2xl + pr-4 sm:pr-8 pour créer un espace propre entre le texte et l'image */}
        <div className="relative grid grid-cols-1 gap-10 items-center z-10 max-w-2xl pr-4 sm:pr-8">
          {/* CONTENEUR ANIMÉ DES PARAGRAPHES */}
          <motion.div
            variants={containerVariants}
            initial="hidden"
            animate="visible"
            className="flex flex-col space-y-6 text-gray-700 leading-relaxed text-base sm:text-lg relative"
          >
            {/* Titre principal */}
            <motion.div variants={paragraphVariants} className="space-y-3">
              <h1
                className={clsx(
                  ...
                  "text-3xl sm:text-4xl lg:text-4xl font-extrabold tracking-tight bg-gradient-to-r from-[#4A154B] via-[#6B21A8] to-[#0D9488] bg-clip-text text-transparent"
                )}
              >
            ...
            {[
              ...
              <motion.p
                ...
                className={clsx(
                  laBelleAurore.className,
                  "text-xl whitespace-pre-line font-bold text-gray-700"
                )}
                style={{
                  textShadow:
            ...
            <motion.div
              ...
              className={clsx(
                laBelleAurore.className,
                "pt-4 flex flex-col items-start space-y-2"
              )}
            >
